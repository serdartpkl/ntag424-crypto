...
class Encoder {
  ...
  static encrypt(masterKey, uid, scanCount, fileData = null, options = {}) {
    ...
    try {
      ...
      // Get and validate SDM profile
      const profile = typeof sdmProfile === 'string' 
        ? SDMConfig.getProfile(sdmProfile)
        : sdmProfile;
      
      if (!profile) {
        throw new ValidationError('Invalid SDM profile', 'sdmProfile', sdmProfile);
      }
      
      // Validate operation with profile
      ...
      // Create secure master key buffer
  ...
  static _buildPiccData(uid, counter, profile) {
    try {
      ...
      if (profile.includeUID && uid) {
        ...
        uid.copy(piccData, uidStart, 0, uidLen);
      }
      
      // Copy counter if included
      if (profile.includeCounter && counter) {
        ...
        counter.copy(piccData, counterStart, 0, counterLen);
      }
